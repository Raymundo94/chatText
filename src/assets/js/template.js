...
function __init__() {
    ...
    if ( !isMobile.any() ) {
        [].forEach.call(document.querySelectorAll('[data-emoji-form]'), function (form) {
            ...
            PICKER_PLUGIN = new EmojiButton({
                position: "top",
                zIndex: 1020
            });

            // PICKER_PLUGIN.on('emoji', function(emoji) {
            //     form.querySelector('[data-emoji-input]').value += emoji;
            // });

            button.addEventListener('click', function () {
                PICKER_PLUGIN.pickerVisible ? PICKER_PLUGIN.hidePicker() : PICKER_PLUGIN.showPicker(button);
            });
        });
    } else {
        [].forEach.call(document.querySelectorAll('[data-emoji-form]'), function (form) {
